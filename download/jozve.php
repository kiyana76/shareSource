...
$db_query_jozve = mysqli_query($connect,"select * from `source_jozveh` where `Source_id`='$source_id'");
$Out_Put = array();
...
$Out_Put_jozve = array();
while ($row = mysqli_fetch_array($db_query_jozve)){
    $Item = array();
    $Item['year'] = $row['year'];
...
<head>
    <meta charset="UTF-8">
    <title>جزوه</title> <!--name jozve bayad beshe-->
...
<div class="content" id="content">
   <img class="img_of_jozve" src="<?php echo "../upload/".$Out_Put[0]['download_img'] ?>">
    <div class="information_jozve" id="informaion_jozve">
        <p> نام جزوه: <?php echo $Out_Put[0]['Course_name'] ?></p>
        <p>نام استاد: <?php echo $Out_Put_jozve[0]['Master_name'] ?></p>
        <p>سال: <?php echo $Out_Put_jozve[0]['year'] ?></p>
        <p>ترم: <?php echo $Out_Put_jozve[0]['term'] ?></p>
