...
$db_query_ketab = mysqli_query($connect,"select * from `source_book` where `Source_id`='$source_id'");
$Out_Put = array();
...
$Out_Put_ketab = array();
while ($row = mysqli_fetch_array($db_query_ketab)){
    $Item = array();
    $Item['Publishers'] = $row['Publishers'];
    $Item['Edit'] = $row['Edit'];
    $Item['Athor'] = $row['Athor'];
    $Out_Put_ketab [] = $Item;
}
?>
...
<head>
    <meta charset="UTF-8">
    <title>کتاب</title><!--bayad name ketab motde nazar bashe-->
    <link rel="stylesheet" href="ketab.css">
...
<div class="content" id="content">
    <img class="img_of_ketab" src="<?php echo "../upload/".$Out_Put[0]['download_img'] ?>">
    <div class="information_ketab" id="informaion_nemone_soal">
        <p>نام کتاب: <?php echo $Out_Put[0]['Course_name'] ?></p>
        <p>نام نویسنده:<?php echo $Out_Put_ketab[0]['Athor'] ?></p>
        <p>انتشارات: <?php echo $Out_Put_ketab[0]['Publishers'] ?></p>
        <p>ویرایش: <?php echo $Out_Put_ketab[0]['Edit'] ?></p>
        <p>تاریخ آپلود: <?php echo $Out_Put[0]['date'] ?></p>
        <p>نام نویسنده: <?php echo $Out_Put[0]['author'] ?></p>
        <div class="show-rate">
            امتیاز:
