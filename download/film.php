...
$db_query_film = mysqli_query($connect,"select * from `source_educational_film` where `Source_id`='$source_id'");
$Out_Put = array();
...
$Out_Put_film = array();
while ($row = mysqli_fetch_array($db_query_film)){
    $Item = array();
    $Item['Master_name'] = $row['Master_name'];
    $Item['Course_Length'] = $row['Course_Length'];
    $Out_Put_film [] = $Item;
}
?>
<!DOCTYPE html>
...
<head>
    <meta charset="UTF-8">
    <title>فیلم آموزشی</title><!--bayad name soal konkor motde nazar bashe-->
    <link rel="stylesheet" href="film%20amoozeshi.css">
...
<div class="content" id="content">
    <img class="img_of_film" src="<?php echo "../upload/".$Out_Put[0]['download_img'] ?>">
    <div class="information_film" id="informaion_film">
        <p>فیلم: <?php echo $Out_Put[0]['Course_name'] ?></p>
        <p>مدرس: <?php echo $Out_Put_film[0]['Master_name'] ?></p>
        <p>مدت زمان: <?php echo $Out_Put_film[0]['Course_Length'] ?></p>
