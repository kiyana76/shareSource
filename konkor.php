...
//***********************************access jozve***********************************
$query_1 = "SELECT * FROM `source` WHERE `Source_type` = 5 AND `validity`=1 ORDER BY id DESC LIMIT 24 ";
$db_query_1 = mysqli_query($connect,$query_1);
$Out_Put_konkor = array();
...
//************************************access filed_name********************
$i = 0;
while ($i < count($Out_Put_konkor)) {
    switch ($Out_Put_konkor[$i]['Group_id']) {
        case 1:
            switch ($Out_Put_konkor[$i]['Filed_id']) {
                case 1:
                    $Out_Put_konkor[$i]['Filed_name'] = 'مهندسی کامپیوتر';
                    break;
                case 2:
                    $Out_Put_konkor[$i]['Filed_name'] = 'مهندسی عمران';
                    break;
                case 3:
                    $Out_Put_konkor[$i]['Filed_name'] = 'آمار';
                    break;
                case 4:
                    $Out_Put_konkor[$i]['Filed_name'] = 'مهندسی برق';
                    break;
                case 5:
                    $Out_Put_konkor[$i]['Filed_name'] = 'مهندسی صنایع';
                    break;
                case 6:
                    $Out_Put_konkor[$i]['Filed_name'] = 'مهندسی مکانیک';
                    break;
            }
            break;
        case 2:
            switch ($Out_Put_konkor[$i]['Filed_id']) {
                case 1:
                    $Out_Put_konkor[$i]['Filed_name'] = 'مدیریت مالی';
                    break;
                case 2:
                    $Out_Put_konkor[$i]['Filed_name'] = 'حسابداری';
                    break;
                case 3:
                    $Out_Put_konkor[$i]['Filed_name'] = 'مدیریت بازرگانی';
                    break;
                case 4:
                    $Out_Put_konkor[$i]['Filed_name'] = ' مدیریت صنعتی';
                    break;
                case 5:
                    $Out_Put_konkor[$i]['Filed_name'] = ' مدیریت فرهنگی هنری';
                    break;
                case 6:
                    $Out_Put_konkor[$i]['Filed_name'] = ' حقوق';
                    break;
                case 7:
                    $Out_Put_konkor[$i]['Filed_name'] = ' روانشماسی';
                    break;
                case 8:
                    $Out_Put_konkor[$i]['Filed_name'] = ' جهانگردی';
                    break;
            }
            break;
        case 3:
            switch ($Out_Put_konkor[$i]['Filed_id']) {
                case 1:
                    $Out_Put_konkor[$i]['Filed_name'] = ' هنرهای تجسمی';
                    break;
                case 2:
                    $Out_Put_konkor[$i]['Filed_name'] = 'ارتباط تصویری';
                    break;
                case 3:
                    $Out_Put_konkor[$i]['Filed_name'] = ' معماری';
                    break;
                case 4:
                    $Out_Put_konkor[$i]['Filed_name'] = 'نقاشی';
                    break;
                case 5:
                    $Out_Put_konkor[$i]['Filed_name'] = ' معماری داخلی';
                    break;
                case 6:
                    $Out_Put_konkor[$i]['Filed_name'] = ' طراحی پارچه و لباس';
                    break;
                case 7:
                    $Out_Put_konkor[$i]['Filed_name'] = ' هنر های اسلامی';
                    break;
            }
            break;
        case 4:
            switch ($Out_Put_konkor[$i]['Filed_id']) {
                case 1:
                    $Out_Put_konkor[$i]['Filed_name'] = 'علوم پایه';
                    break;
            }
            break;
    }
    ...
    <div id="main_content">
        ...
        <div id="recommended_jozve">
            <ul id="jadvale_recommended_jozve">  <!--jadvale jozavate pishnahadie-->
                <?php $i = 0;
                if (count($Out_Put_konkor) != 0) {
                    while ($i < count($Out_Put_konkor)) { ?>
                        <li class="list_result">
                            <div>
                                <img class="img_recommend" src="<?php echo 'upload/' . $Out_Put_konkor[$i]['Download_img']; ?>">
                                <a href="download/<?php echo $Out_Put_konkor[$i]['Source_Type_Name'] ?>.php?id=<?php echo $Out_Put_konkor[$i]['source_id'] ?>">
                                    <?php echo $Out_Put_konkor[$i]['Course_name'] ?></a>
                                <p> تاریخ انتشار:<?php echo $Out_Put_konkor[$i]['date'] ?></p>
                                <p>نام منتشرکننده:<?php echo $Out_Put_konkor[$i]['author'] ?></p>
                                <p>رشته:<?php echo $Out_Put_konkor[$i]['Filed_name'] ?></p>
                            </div>
                        </li>
                        <?php $i++;
                    }
