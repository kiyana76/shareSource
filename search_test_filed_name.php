...
$db_Query = mysqli_query($connect  , "SELECT *  FROM `course_filed` WHERE `Filed_name`  LIKE   '%$Search_text%';");
...
{
    $filed = $Out_Put[$i]['Filed_id'];
    $group = $Out_Put[$i]['Group_id'];
    $query_source = "SELECT * FROM `source` WHERE `Filed_id` = '$filed' AND `Group_id` = '$group' AND `validity`=1;";
        ...
        if(count($Out_Put_source) != 0) {
            while ($i < count($Out_Put_source)) { ?>
                <li class="list_result">
                    <div>
                        <img class="img_recommend" src="<?php echo 'upload/' . $Out_Put_source[$i]['Download_img'];?>">
                        <a href="download/<?php echo $Out_Put_source[$i]['Source_Type_Name']?>.php?id=<?php echo $Out_Put_source[$i]['source_id']?>"><?php echo $Out_Put_source[$i]['Course_name'] ?></a>
