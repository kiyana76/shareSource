...
if (in_array($extension, $allowedExts)) { //check kardane inke file upload shode dar pasvand haye mojaz bashad.
    ...
    move_uploaded_file($_FILES["file"]["tmp_name"],
        "upload/nemone_soal/" . $_FILES["file"]["name"]);
    /*echo "Stored in: " . "upload/nemone_soal/" . $_FILES["file"]["name"];*/
...
function allocateName($ext)
{
    ...
    if (file_exists("upload/nemone_soal/" . $_FILES["file"]["name"])) {
        return false;
    } else {
        return true;
    }
...
if (in_array($extension, $allowedExtsImg)) { //check kardane inke file upload shode dar pasvand haye mojaz bashad.
    ...
    move_uploaded_file($_FILES["image"]["tmp_name"],
        "upload/nemone_soal_img/" . $_FILES["image"]["name"]);
    /*echo "Stored in: " . "upload/nemone_soal_img/" . $_FILES["image"]["name"];*/
...
function allocateNameImg($ext)
{
    ...
    if (file_exists("upload/nemone_soal_img/" . $_FILES["image"]["name"])) {
        return false;
    } else {
        return true;
    }
...
$validity = 0;
$Rate_count = 0;
$Download_Link = "upload/nemone_soal/". $_FILES["file"]["name"];
$Download_img = "upload/nemone_soal_img/" . $_FILES["image"]["name"];
