...
if (isset($_REQUEST['source_name'])   &&  isset($_REQUEST['Master_name']) && isset($_REQUEST['Course_Length']) && isset($_REQUEST['Group_id']) && isset($_REQUEST['Filed_id']) ){
    $source_name= $_REQUEST['source_name'];
    $Master_name = $_REQUEST['Master_name'];
    $Course_Length= $_REQUEST['Course_Length'];
...
//**************************************upload file**********************************
$allowedExts = array("m4v","mp4","wmv","mov","avi","mpg","mpeg","M4V","MP4","WMV","MOV","AVI","MPG","MPEG"); //faghat file haye ghabele upload
...
if (in_array($extension, $allowedExts)) { //check kardane inke file upload shode dar pasvand haye mojaz bashad.
    ...
    move_uploaded_file($_FILES["file"]["tmp_name"],
        "upload/video/" . $_FILES["file"]["name"]);
    /*echo "Stored in: " . "upload/video/" . $_FILES["file"]["name"];*/
...
if (in_array($extension, $allowedExtsImg)) { //check kardane inke file upload shode dar pasvand haye mojaz bashad.
    ...
    move_uploaded_file($_FILES["image"]["tmp_name"],
        "upload/video_img/" . $_FILES["image"]["name"]);
    /*echo "Stored in: " . "upload/video_img/" . $_FILES["image"]["name"];*/
...
function allocateNameImg($ext)
{
    ...
    if (file_exists("upload/video_img/" . $_FILES["image"]["name"])) {
        return false;
    } else {
        return true;
    }
...
$validity = 0;
$Rate_count = 0;
$Download_Link = "upload/video/". $_FILES["file"]["name"];
$Download_img = "upload/video_img/" . $_FILES["image"]["name"];
...
if($Result == 1){
    ...
    if ( $S_Type== 4  ){
        ...
        $in_Course_Length  = $_REQUEST['Course_Length'];
        $in_Master_name = $_REQUEST['Master_name'];

        $query_4 = "UPDATE `source_educational_film`
			SET `Course_Length` = '$in_Course_Length',  `Master_name` = '$in_Master_name' 
			WHERE `Source_id`='$N_Source_id';";

        $db_Query_4 = mysqli_query($connect  , $query_4);

        if ($connect->query($query_4)===  true) {
            echo " ";
        }else{
            echo "error    "  .$connect->error ;
        }
