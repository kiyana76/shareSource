...
if (isset($_REQUEST['source_name'])   &&  isset($_REQUEST['Athor']) && isset($_REQUEST['Edit']) && isset($_REQUEST['Publishers']) && isset($_REQUEST['Group_id']) && isset($_REQUEST['Filed_id']) ){
...
if (in_array($extension, $allowedExts)) { //check kardane inke file upload shode dar pasvand haye mojaz bashad.
    ...
    move_uploaded_file($_FILES["file"]["tmp_name"],
        "upload/ketab/" . $_FILES["file"]["name"]);
    /*echo "Stored in: " . "upload/ketab/" . $_FILES["file"]["name"];*/
...
function allocateName($ext)
{
    ...
    if (file_exists("upload/ketab/" . $_FILES["file"]["name"])) {
        return false;
    } else {
        return true;
    }
...
if (in_array($extension, $allowedExtsImg)) { //check kardane inke file upload shode dar pasvand haye mojaz bashad.
    ...
    move_uploaded_file($_FILES["image"]["tmp_name"],
        "upload/ketab_img/" . $_FILES["image"]["name"]);
    /*echo "Stored in: " . "upload/ketab_img/" . $_FILES["image"]["name"];*/
...
function allocateNameImg($ext)
{
    ...
    if (file_exists("upload/ketab_img/" . $_FILES["image"]["name"])) {
        return false;
    } else {
        return true;
    }
...
$validity = 0;
$Rate_count = 0;
$Download_Link = "upload/ketab/". $_FILES["file"]["name"];
$Download_img = "upload/ketab_img/" . $_FILES["image"]["name"];
...
if($Result == 1){
    ...
    if ( $S_Type== 2  ){
        ...
        $in_Publishers = $_REQUEST['Publishers'];
        $in_Edit = $_REQUEST['Edit'];
        $in_Athor = $_REQUEST['Athor'];

        $query_2 = "UPDATE `source_book`
			SET `Publishers` = '$in_Publishers' , `Edit` = '$in_Edit',  `Athor` = '$in_Athor' 
			WHERE `Source_id`='$N_Source_id';";

        $db_Query_2 = mysqli_query($connect  , $query_2);

        if ($connect->query($query_2)===  true) {
            echo " ";
        }else{
            echo "error    "  .$connect->error ;
        }
