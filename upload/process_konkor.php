...
session_start();
/*$_SESSION["First_name"] = 'kiyana';
$_SESSION["Last_name"] = 'kiyana';*/
...
if (isset($_REQUEST['source_name']) && isset($_REQUEST['year']) && isset($_REQUEST['Group_id']) && isset($_REQUEST['Filed_id']) ){
...
if (in_array($extension, $allowedExts)) { //check kardane inke file upload shode dar pasvand haye mojaz bashad.
    ...
    move_uploaded_file($_FILES["file"]["tmp_name"],
        "upload/konkor/" . $_FILES["file"]["name"]);
    /*echo "Stored in: " . "upload/konkor/" . $_FILES["file"]["name"];*/
...
function allocateName($ext)
{
    ...
    if (file_exists("upload/konkor/" . $_FILES["file"]["name"])) {
        return false;
    } else {
        return true;
    }
...
if (in_array($extension, $allowedExtsImg)) { //check kardane inke file upload shode dar pasvand haye mojaz bashad.
    ...
    move_uploaded_file($_FILES["image"]["tmp_name"],
        "upload/konkor_img/" . $_FILES["image"]["name"]);
    /*echo "Stored in: " . "upload/konkor_img/" . $_FILES["image"]["name"];*/
...
function allocateNameImg($ext)
{
    ...
    if (file_exists("upload/konkor_img/" . $_FILES["image"]["name"])) {
        return false;
    } else {
        return true;
    }
...
$validity = 0;
$Rate_count = 0;
$Download_Link = "upload/konkor/". $_FILES["file"]["name"];
$Download_img = "upload/konkor_img/" . $_FILES["image"]["name"];
...
if($Result == 1){
    ...
    if ( $S_Type== 5  ){
        ...
        $query_5 = "UPDATE `source_entrance_exam`
			SET `year` = '$in_year'  
			WHERE `Source_id`='$N_Source_id';";

        $db_Query_5 = mysqli_query($connect  , $query_5);

        if ($connect->query($query_5)===  true) {
            echo " ";
        }else{
            echo "error    "  .$connect->error ;
        }
